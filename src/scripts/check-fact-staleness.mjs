...
async function main() {
  ...
  // carrierFindings carries no standing rows: an unchecked cell is never reported here,
  // so every carrier-roaming finding is an aged one.
  ...
  const total = findings.length + noKf.length + spokeFindings.length + heroFindings.length
    + formFindings.length + mapFindings.length + carrierFindings.length;
