...
,
  ...
  spokes: [
    ...
    },
    {
      ...
      description: `Coverage is strong in cities, on the coast and in main towns, thin in the remote Outback. US roaming runs about ${S.dayPass}; an eSIM is less.`,
      ...
      sections: [
        {
          ...
          icon: "roamfee",
          key: { fig: `${S.dayPassFig}`, tag: "Roaming default", text: `A US carrier day pass runs about ${S.dayPassGlance}, so a two-week Australia trip is real money. T-Mobile is the exception, with ${S.tmoRange} of high-speed data abroad on its current mid and upper tiers, so check yours before buying.`, tone: "teal" },
          p: [
            `Australia is a long-trip destination, which makes the roaming math worse than it looks. <b>AT&amp;T and Verizon</b> day passes run about <b>${S.dayPass}</b>, so a week is around <b>${S.week}</b> and the two or three weeks most people spend here turns into a few hundred dollars for data you can buy far cheaper. <b>T-Mobile</b> is the exception worth checking first: its current tiers include a genuine <b>high-speed allowance</b> abroad, roughly <b>${S.tmoLow}</b> on Experience More, Magenta MAX and Go5G Plus, <b>${S.tmoMid}</b> on Experience Beyond and Go5G Next, and <b>${S.tmoHigh}</b> on Better Value, slowing to <b>${S.throttle}</b> only once that is used up. The entry <b>Essentials</b> tier is the one throttled from the start. Google Fi and Visible set their own terms, so read your own plan rather than assuming.`,
            `The trap to avoid entirely is roaming with <b>no plan</b>. Pay-per-use data runs about <b>${S.ppu}</b>, thousands per gigabyte, and phones use data in the background without being asked, so <b>turn off data roaming</b> until your eSIM or pass is live. <b>Physical prepaid SIMs</b> are easy to buy in Australia at airports, supermarkets and phone shops if you prefer one, but for a normal trip an eSIM saves you the errand on your first jet-lagged morning.`
          ]
        },
        {
      ...
      faqs: [
        { q: "Is an eSIM cheaper than roaming in Australia?", a: `Usually by a wide margin, though the gap stops growing sooner than you might think. A Nomad eSIM is a few dollars per gigabyte, against about ${S.dayPass} for an AT&T or Verizon roaming pass, so a two-week trip runs about ${S.capAmount} on AT&T, which caps its Day Pass at ${S.capFees}. An eSIM still costs a fraction of that, and past day 10 on AT&T the daily arithmetic no longer applies.` },
