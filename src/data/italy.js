...
export default {
  ...
  spokes: [
    ...
    },
    {
      ...
      sections: [
        {
          ...
          icon: "roamfee",
          key: { fig: `${S.dayPassFig}`, tag: "Roaming default", text: `Coverage is good where most people go, so this is mainly about cost. A US carrier day pass runs about ${S.dayPassGlance}, and T-Mobile is the exception, with ${S.tmoRange} of high-speed data abroad on its current mid and upper tiers, so check yours before buying.`, tone: "teal" },
          p: [
            `Italy's mobile coverage is good across the places most people go, so the main decision is how you pay. The expensive default is roaming on your US plan: <b>AT&amp;T and Verizon</b> charge about <b>${S.dayPass}</b>, so a week runs <b>${S.week}</b>. <b>T-Mobile</b> is the exception worth checking first: its current tiers include a genuine <b>high-speed allowance</b> abroad, roughly <b>${S.tmoLow}</b> on Experience More, Magenta MAX and Go5G Plus, <b>${S.tmoMid}</b> on Experience Beyond and Go5G Next, and <b>${S.tmoHigh}</b> on Better Value, slowing to <b>${S.throttle}</b> only once that is used up. The entry <b>Essentials</b> tier is the one throttled from the start. Google Fi and Visible set their own terms, so read your own plan rather than assuming.`,
            `Whatever you choose, do not roam with <b>no plan</b>. Pay-per-use data is around <b>${S.ppu}</b>, thousands per gigabyte, and phones use data in the background, so <b>turn off data roaming</b> until an eSIM or pass is active.`
          ]
        },
        {
      ...
      faqs: [
        { q: "Do I need an eSIM for Italy?", a: `For most US travelers it is the cheapest way to get real data. Italy's coverage is strong in cities and along the coast, so it is a cost question: a Nomad eSIM is a few dollars per gigabyte, versus about ${S.dayPass} for an AT&T or Verizon roaming pass. Set it up before you fly and it works from landing.` },
