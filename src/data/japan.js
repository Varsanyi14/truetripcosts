...
export default {
  ...
  spokes: [
    ...
    },
    {
      ...
      description: `Japan has some of the best coverage anywhere, bullet trains included, so this is purely cost. US roaming runs ${S.dayPass}; an eSIM is less.`,
      ...
      sections: [
        {
          ...
          icon: "roamfee",
          key: { fig: `${S.dayPassFig}`, tag: "Roaming default", text: `Coverage is a non-issue in Japan, so the only question is what you pay. A US carrier day pass runs about ${S.dayPassGlance}, and T-Mobile is the exception, with ${S.tmoRange} of high-speed data abroad on its current mid and upper tiers, so check yours before buying.`, tone: "teal" },
          p: [
            `Because coverage is not a problem here, the whole decision is price. Roaming on your US plan is the expensive default: <b>AT&amp;T and Verizon</b> day passes run about <b>${S.dayPass}</b>, so a week is roughly <b>${S.week}</b>. Past that, <b>AT&amp;T caps the Day Pass at ${S.capFees}</b>, about <b>${S.capAmount}</b>, and covers the rest of that bill period on land, so a two-week trip is nearer 120 than the 170 a straight daily multiplication suggests. A trip straddling two bill periods can meet the cap twice. <b>T-Mobile</b> is the exception worth checking first: its current tiers include a genuine <b>high-speed allowance</b> abroad, roughly <b>${S.tmoLow}</b> on Experience More, Magenta MAX and Go5G Plus, <b>${S.tmoMid}</b> on Experience Beyond and Go5G Next, and <b>${S.tmoHigh}</b> on Better Value, slowing to <b>${S.throttle}</b> only once that is used up. The entry <b>Essentials</b> tier is the one throttled from the start. Google Fi and Visible set their own terms, so read your own plan rather than assuming. That matters more in Japan than in most places, because you will lean on <b>maps and transit apps</b> constantly to navigate a rail network with more exits per station than some towns have streets, and a throttled ${S.throttle} connection handles that badly.`,
            `The thing not to do is roam with <b>no plan</b>. Pay-per-use data runs about <b>${S.ppu}</b>, thousands per gigabyte, and background app activity alone can run a bill up before you have opened anything, so <b>turn off data roaming</b> until an eSIM or a pass is active.`
          ]
        },
        {
